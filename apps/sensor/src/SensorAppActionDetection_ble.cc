...
namespace apps {
  namespace sensor {
    ...
    std::deque<uint32_t> ACTION_QUEUE; // 他のファイルでも同じ変数名で宣言してしまうと重複エラーが出る (staticをつければ大丈夫)
    ...
    void SensorApp::callback_ActionDetection(uint32_t classification, float probability){
      ...
      if(action_result == ACTION_CLASSIFICATION_STABLE){
        ...
        if(g_ringtone_status > 0 && g_ringtone_status != 1){
          StopGuidance();
          std::this_thread::sleep_for(std::chrono::milliseconds(200));
          TWSS_RINFO(APPNAME"::%s Guidance Stop", __func__);
        }
        if(g_ringtone_status != 1){
        ...
        if(g_ringtone_status > 0 && g_ringtone_status != 2){
          StopGuidance();
          std::this_thread::sleep_for(std::chrono::milliseconds(200));
          TWSS_RINFO(APPNAME"::%s Guidance Stop", __func__);
        }
        if(g_ringtone_status != 2){
